...
export default function KeysPage() {
  ...
  async function issueKey(e: React.FormEvent) {
    e.preventDefault()
    if (!issueForm.key_id || !issueForm.issued_to.trim()) { alert(t('keys.keyIssuedToRequired')); return }
  ...
  async function addKey(e: React.FormEvent) {
    e.preventDefault()
    if (!newKey.key_name.trim()) { alert(t('keys.keyNameRequired')); return }
  ...
  async function deleteKey(id: number) {
    if (!confirm(t('keys.confirmDeleteKey'))) return
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h1 className="text-3xl font-bold flex items-center gap-2"><KeyRound className="w-7 h-7 text-blue-600" /> {t('keys.title')}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.3fr] gap-6 mb-8">
        <form onSubmit={issueKey} className="bg-white border rounded-xl shadow-sm p-6 space-y-3 h-fit">
          <h2 className="text-sm font-bold text-slate-700 mb-1">{t('keys.issueKey')}</h2>
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">{t('keys.key')}</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">{t('keys.issuedTo')}</label>
          ...
          <button type="submit" className="w-full bg-blue-600 text-white text-sm font-semibold px-4 py-2.5 rounded-lg hover:bg-blue-700 mt-2">{t('keys.issueBtn')}</button>
        </form>

        <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b bg-gray-50"><h2 className="text-sm font-bold text-slate-700">{t('keys.currentlyOut')} ({outLogs.length})</h2></div>
            ...
            {outLogs.map(l => (
              <div key={l.id} className="px-4 py-3 flex items-center justify-between text-sm">
                <div>
                  ...
                  <div className="text-xs text-gray-500">{l.purpose || '-'} · {t('keys.issued')} {new Date(l.time_issued).toLocaleString()}</div>
                </div>
                <button onClick={() => returnKey(l)} className="text-xs bg-green-50 text-green-700 px-2.5 py-1.5 rounded-lg hover:bg-green-100 flex-shrink-0">{t('keys.markReturned')}</button>
              </div>
            ))}
            {outLogs.length === 0 && <p className="px-4 py-8 text-center text-sm text-gray-400">{t('keys.noKeysOut')}</p>}
          </div>
        </div>
      </div>
      ...
      {isManager && (
        <div className="bg-white border rounded-xl shadow-sm p-6 mb-8">
          <h2 className="text-sm font-bold text-slate-700 mb-3">{t('keys.keyDefinitions')}</h2>
          <form onSubmit={addKey} className="flex flex-wrap gap-2 items-end mb-4">
            <input placeholder={t('keys.keyName')} value={newKey.key_name} onChange={e => setNewKey({ ...newKey, key_name: e.target.value })} className="border rounded-md px-3 py-2 text-sm" />
            <input placeholder={t('keys.keyNo')} value={newKey.key_no} onChange={e => setNewKey({ ...newKey, key_no: e.target.value })} className="border rounded-md px-3 py-2 text-sm" />
            <input placeholder={t('keys.description')} value={newKey.description} onChange={e => setNewKey({ ...newKey, description: e.target.value })} className="border rounded-md px-3 py-2 text-sm flex-1 min-w-[150px]" />
          ...
          <div className="divide-y divide-gray-100">
            ...
            {keys.length === 0 && <p className="text-sm text-gray-400 py-2">{t('keys.noKeysDefined')}</p>}
          </div>
        </div>
      )}

      <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b bg-gray-50"><h2 className="text-sm font-bold text-slate-700">{t('keys.recentActivity')}</h2></div>
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{t('keys.key')}</th>
              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{t('keys.issuedTo')}</th>
              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{t('keys.issuedCol')}</th>
              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{t('keys.returnedCol')}</th>
            ...
            {logs.map(l => (
              <tr key={l.id}>
                ...
                <td className="px-4 py-2">
                  <span className={`text-xs font-bold uppercase rounded-full px-2 py-0.5 ${l.status === 'out' ? 'bg-amber-50 text-amber-700' : 'bg-green-50 text-green-700'}`}>{t(`status.${l.status}`)}</span>
                </td>
              </tr>
            ))}
            {logs.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">{t('keys.noActivity')}</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
